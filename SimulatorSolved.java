...
public class SimulatorSolved {
	...
	*/
	...
	public static void main(String[] args){
		SimulatorSolved sim = new SimulatorSolved();
		sim.dispatcher(args);
    ...
    */
    void dispatcher(String[] args) {
		...
		* セマフォ
		*/
		Semaphore waiter = new Semaphore(getNum(),true);

		// type of callee class
		Class<?>[] types = {int.class, MessageQueue.class,Semaphore.class};
		...
		try{
			for (int i = 0; i < getNum(); i++) {
				...
				Future<?> future = exec.submit((ProcessSolved)cnst.newInstance(arg));
				list.add(future);
